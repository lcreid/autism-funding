...
module MyProfileTestHelper
  class MyProfileTestPage < CapybaraTest
    ...
    def set_invalid(list)
      unless list.respond_to? :include?
        list = [list]
      end
      @items.each do |key,item|
        if list.include? key
  ...
  class FundedPersonTest < ActiveSupport::TestCase
    ...
    def check_the_kid(frm_index=0)
      set_ids(frm_index)
      ret = self.tb_name_first.check_input
      ret == '' && ret = self.tb_name_middle.check_input
      ret == '' && ret = self.tb_name_last.check_input
      ret == '' && ret = self.tb_birthdate.check_input
      return ret
  ...
  class ATextBox < ActiveSupport::TestCase
    ...
    def initialize(init_params)
      unless init_params.class.to_s == 'Hash'
        init_params = {}
      end
    ...
    def check_input()
      ...
      if self.is_valid
        ...
        unless self.present_value == self.expected_value
          show_me __LINE__
          ret = "Unexpected (Valid) contents expecting: [#{self.expected_value}], was [#{self.present_value}]"
        end
# if self.id == 'user_funded_people_attributes_0_birthdate'
#   show_me __LINE__
# end
        if ret == '' && @chk_validator
          if chk_for_error_message?(self.id, self.err_message)
        ...
        unless self.present_value == self.expected_value
          show_me __LINE__
          ret = "Unexpected (InValid) contents expecting: #{self.expected_value}, was #{self.present_value}"
        end
        if ret == '' && @chk_validator
          if ! chk_for_error_message?(self.id, self.err_message)
    ...
    def expected_value
      if @is_valid
        ev = @formatted_valid_value
      else
        ev = @formatted_invalid_value
      end
      if ev.nil?
        ev = ''
      end
...
#      puts "-------#{@id}----------------"
    ...
    def show_me(the_line=0)
      ...
      puts "--  expected:            val: [#{self.expected_value}] class: #{self.expected_value.class}"
      puts "--     value:            val: [#{self.present_value}]  class: #{self.present_value.class}"
      puts "----------------------------------------------"
    end #def show_me
